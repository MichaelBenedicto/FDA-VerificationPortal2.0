...
export default function WPS_CDRRHR_Dashboard() {
  ...
  const fetchRecords = async (pageNumber = 1, searchValue = search) => {
    setLoading(true);
    try {
      const res = await axios.get("/fda/cdrrhr/wps", {
        params: {
          search: searchValue,
          page: pageNumber,
        },
  ...
  const handleImportChange = async (e) => {
    ...
    if (window.confirm(`Are you sure you want to import data from ${file.name}?`)) {
      ...
      try {
        setLoading(true);
        await axios.post("/fda/cdrrhr/wps/import", formData, {
  ...
  const handleSubmit = async () => {
    let localErrors = {};
    if (!form.registration_number) localErrors.registration_number = "WPS Number is required";
    ...
    try {
      const confirmMsg = editData
        ? "Do you want to update this WPS Details?"
        : "Do you want to add this WPS Details?";

      if (!window.confirm(confirmMsg)) return;

      if (editData) {
        await axios.post(`/fda/cdrrhr/wps/update/${editData.registration_number}`, form);
        showNotification("success", "WPS Details updated successfully!");
      } else {
        await axios.post("/fda/cdrrhr/wps/add", form);
        showNotification("success", "Medical Device WPS added successfully!");
      }
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center mb-4 gap-2">
        <input
          type="text"
          placeholder="Search Medical Device WPS..."
      ...
      {modalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-xl shadow-lg w-96 max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold mb-4">{editData ? "Edit Medical Device WPS" : "Add Medical Device WPS"}</h2>
