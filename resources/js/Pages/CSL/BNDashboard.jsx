...
export default function BNDashboard() {
  ...
  const fetchNotifications = async (pageNumber = 1, searchValue = search) => {
    setLoading(true);
    try {
      const res = await axios.get("/fda/csl/batch-notifications", {
        params: {
          search: searchValue,
          page: pageNumber,
        },
  ...
  const openEditModal = (row) => {
    setEditData(row);
    setForm({
      batch_notification_number: row.batch_notification_number,
  ...
  const handleImportChange = async (e) => {
    ...
    if (window.confirm(`Are you sure you want to import data from ${file.name}?`)) {
      ...
      try {
        setLoading(true);
        await axios.post("/fda/csl/batch-notifications/import", formData, {
  ...
  const handleSubmit = async () => {
    let localErrors = {};
    if (!form.batch_notification_number) localErrors.batch_notification_number = "BN Number is required";
    ...
    try {
      const confirmMsg = editData
        ? "Do you want to update this batch notification?"
        : "Do you want to add this batch notification?";

      if (!window.confirm(confirmMsg)) return;

      if (editData) {
        await axios.post(`/fda/csl/batch-notifications/update/${editData.batch_notification_number}`, form);
        showNotification("success", "Batch notification updated successfully!");
      } else {
        await axios.post("/fda/csl/batch-notifications/add", form);
        showNotification("success", "Batch notification added successfully!");
      }
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center mb-4 gap-2">
        <input
          type="text"
          placeholder="Search Batch Notification..."
        ...
        <div className="overflow-auto max-h-[600px] bg-white rounded-xl shadow border">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50 sticky top-0 z-10">
              <tr>
                <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">BN Number</th>
                ...
                <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Batch No.</th>
                <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Lot No.</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {records.length > 0 ? (
                records.map((row) => (
                  <tr key={row.batch_notification_number}>
                    <td className="px-4 py-2 font-medium">{row.batch_notification_number}</td>
                    ...
                    <td className="px-4 py-2">{row.batch_number}</td>
                    <td className="px-4 py-2">{row.lot_number}</td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-xl shadow-lg w-96 max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold mb-4">{editData ? "Edit Notification" : "Add Notification"}</h2>

            <div className="mb-2">
              <label className="text-xs font-semibold text-gray-600">Batch Notification Number</label>
              <input
                type="text"
                placeholder="BN-YYYY-XXXX"
                disabled={!!editData}
                className={`border p-2 w-full rounded ${errors.batch_notification_number ? "border-red-500" : ""} ${
                  editData ? "bg-gray-100 text-gray-500 cursor-not-allowed" : ""
                }`}
                value={form.batch_notification_number}
                onChange={(e) => {
                  setForm({ ...form, batch_notification_number: e.target.value });
                  if (errors.batch_notification_number) setErrors({ ...errors, batch_notification_number: null });
                }}
              />
              {errors.batch_notification_number && <p className="text-red-500 text-xs mt-1">{errors.batch_notification_number}</p>}
            </div>
            ...
            <div className="mb-2">
              <label className="text-xs font-semibold text-gray-600">Lot Number</label>
              <input
                type="text"
