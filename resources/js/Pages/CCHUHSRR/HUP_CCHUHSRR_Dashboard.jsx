...
export default function HUP_CCHUHSRR_Dashboard() {
  ...
  const fetchRecords = async (pageNumber = 1, searchValue = search) => {
    setLoading(true);
    try {
      const res = await axios.get("/fda/cchuhsrr/hup_cpr", {
        params: {
          search: searchValue,
          page: pageNumber,
        },
      ...
      const errorMsg =
        err.response?.data?.message || err.message || "Failed to load HUP CPR.";
  ...
  const handleImportChange = async (e) => {
    ...
    if (window.confirm(`Are you sure you want to import data from ${file.name}?`)) {
      ...
      try {
        setLoading(true);
        await axios.post("/fda/cchuhsrr/hup_cpr/import", formData, {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editData) {
        await axios.post(`/fda/cchuhsrr/hup_cpr/update/${editData.registration_number}`, form);
        showNotification("success", "CPR Details updated successfully!");
      } else {
        await axios.post("/fda/cchuhsrr/hup_cpr/add", form);
        showNotification("success", "HUP CPR added successfully!");
      }
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50 sticky top-0 z-10">
              <tr>
                ...
                <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Packaging</th>
                <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Active Ingredient</th>
                <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Intended Use</th>
                ...
                <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Cancellation</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {records.length > 0 ? (
                records.map((row, idx) => (
                  <tr key={row.registration_number || idx}>
                    <td className="px-4 py-2 font-medium">{row.registration_number}</td>
                    <td className="px-4 py-2">{row.product_name}</td>
                    <td className="px-4 py-2">{row.packaging}</td>
                    <td className="px-4 py-2">{row.active_ingredient}</td>
                    <td className="px-4 py-2">{row.intended_use}</td>   
      ...
      {modalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-xl shadow-lg w-96 max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold mb-4">{editData ? "Edit HUP CPR" : "Add HUP CPR"}</h2>
            ...
            <div className="mb-2">
              <label className="text-xs font-semibold text-gray-600">Packaging</label>
              <input
                type="text"
                placeholder="Packaging"
                className={`border p-2 w-full rounded ${errors.packaging ? "border-red-500" : ""}`}
                value={form.packaging}
                onChange={(e) => {
                  setForm({ ...form, packaging: e.target.value });
                  if (errors.packaging) setErrors({ ...errors, packaging: null });
                }}
              />
            ...
            <div className="mb-2">
              <label className="text-xs font-semibold text-gray-600">Active Ingredient</label>
              <input
                type="text"
                placeholder="Active Ingredient"
                className={`border p-2 w-full rounded ${errors.active_ingredient ? "border-red-500" : ""}`}
                value={form.active_ingredient}
                onChange={(e) => {
                  setForm({ ...form, active_ingredient: e.target.value });
                  if (errors.active_ingredient) setErrors({ ...errors, active_ingredient: null });
                }}
              />
              {errors.active_ingredient && <p className="text-red-500 text-xs mt-1">{errors.active_ingredient}</p>}
            </div>
            ...
            <div className="mb-2">
              ...
              <input
                ...
                className={`border p-2 w-full rounded ${errors.intended_use ? "border-red-500" : ""}`}
                value={form.intended_use}
                onChange={(e) => {
                  setForm({ ...form, intended_use: e.target.value });
                  if (errors.intended_use) setErrors({ ...errors, intended_use: null });
                }}
              />
              {errors.intended_use && <p className="text-red-500 text-xs mt-1">{errors.intended_use}</p>}
            </div>
            ...
            <div className="mb-4">
              <label className="text-xs font-semibold text-gray-600 block mb-1">IS_Canceled</label>
              <select
                className="border p-2 w-full rounded"
